...
class Trader:
    ...
    def broadcast(self, sole_trader_message="Solo Trader"):
        ...
        print(f"Trader {self.trader_id}: Broadcasting solo trader message to all addresses: {sole_trader_message}")
        print(f"Trader {self.trader_id}: Current clients: {self.all_clients}")

        for address in self.all_clients:
            try:
                print(f"Trader {self.trader_id}: Attempting to connect to {address}")
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    ...
                    print(f"Trader {self.trader_id}: Broadcasted message to {address}")
            except Exception as e:
                print(f"Trader {self.trader_id}: Failed to broadcast message to {address}: {e}")
        ...
        while (self.running and self.other_trader_alive):
            ...
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    ...
                    print(f"Trader {self.trader_id} Sent heartbeat message.")

                    # Try to receive the response
                    try:
                        data = s.recv(1024).decode()
                        if data == HEARTBEAT_RESPONSE:
                            print(f"Trader {self.trader_id} received heartbeat response {data}.")
                            pass
                        else:
                        ...
                        print(f"Trader {self.trader_id}: No heartbeat response from {backup_host}:{backup_port}. Assuming down.")
                        self.other_trader_alive = False
                        self.broadcast()
                        # Implement failover logic if needed
            except ConnectionRefusedError:
                # Could not connect, other trader likely down
                print(f"Trader {self.trader_id}: Cannot connect to {backup_host}:{backup_port}. Assuming down.")
                self.other_trader_alive = False
                self.broadcast()
                # Implement failover logic if needed
            except Exception as e:
                print(f"Trader {self.trader_id} heartbeat encountered error: {e}")
        ...
        try:
            while True:
                ...
                if data == HEARTBEAT_MESSAGE:
                    ...
                    print(f"Trader {self.trader_id} received and responded to heartbeat message {data}.")
                    continue
            ...
            print(f"Trader {self.trader_id}: Connection reset by {address}")
        except Exception as e:
            print(f"Trader {self.trader_id} encountered an error with {address}: {e}")
        finally:
            client_socket.close()
