...
class Peer:
    def __init__(self, peer_id, role, neighbors, port, leader=None, ip_address='localhost', item=None):
    ...
    def send_update_inventory(self):
        ...
        with self.operations_lock:
            ...
            if self.leader is None:
                ...
                while self.leader is None and self.running:
                    time.sleep(1)
                if not self.running:
                    return
    ...
    def process_pending_buy_requests(self):
        ...
        while True:
            ...
            self.execute_buy_request(message)
    ...
    def handle_buy_confirmation(self, message):
        ...
        if confirmation_message.buyer_id == self.peer_id:
            if confirmation_message.status:
                ...
                threading.Timer(wait_time, self.buy_item).start()
    ...
    def declare_leader(self):
        ...
        # Update global leader reference if needed
        global leader
        leader = self
        self.in_election = False
    ...
    def handle_leader(self, message):
        ...
        if leader_id != self.peer_id:
            ...
            if self.leader is None:
                ...
                self.leader = Peer(leader_id, {'leader'}, [], message['port'], ip_address=message['ip_address'])
        else:
            self.is_leader = True
            self.leader = self
            # Initialize leader-specific attributes
